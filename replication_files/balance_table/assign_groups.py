...
def assign_groups(df_ppl):
    ...
    (Original function from your script)
    '''
    conditions = ['information', 'control', 'saliency']
    ...
    df_ppl = df_ppl.sort_values(by='total_time_spent').reset_index(drop=True) # Ensure sorted and index reset
    ...
    for i in range(0, N_rows - (N_rows % 3), 3):
        ...
        for j, cond_idx in enumerate(block_indices):
            ...
            df_ppl.loc[cond_idx, 'block_idx'] = i
...
def check_balance(cur_df):
    ...
    info_df = merged_df[merged_df['experiment_condition'] == 'information']
